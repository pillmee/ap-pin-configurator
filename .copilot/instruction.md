...
- UI
  ...
  - 필터 기능
    - 테이블의 내용을 검색하여 필터링된 내용을 테이블에 보여주는 기능을 추가한다.
    - "Ball Location"과 "Signal Name"을 검색 대상으로 한다.
    - "Signal Name"을 필터링할 때는 콤보박스에 보이지 않는 항목까지 검색 대상으로 삼아야 한다.
  - UI 스타일
    - 테이블에 로딩되는 내용이 많아 가능한 화면 레이아웃을 컴팩트하게 구성한다.
